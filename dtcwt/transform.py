...
class DTCWTForward(nn.Module):
    """ Performs a 2d & 3d DTCWT Forward decomposition of an image
    Mostly Authored by: fbcotter
    Args:
    ...
    def forward(self, x):
        ...
        # Modify: add 3D input
            ...
            # Padding!
            # If the row/col count of X is not divisible by 2 in any dim then we need to
            # extend X
            d, r, c = x.shape[2:]
            if c % 2 != 0:
                x = torch.cat((x, x[..., -1:]), dim=-1)
            ...
            # Do the level 1 transform, get the (equal-size)low and (2-downsample)highpass outputs
            # h0o is lowpass-filter, h1o is highpass-filter
            low, h = FWD_J1_3D.apply(x, self.h0o, self.h1o, self.skip_hps[0],
                                self.o_dim, self.ri_dim, mode)
            highs[0] = h
            if self.include_scale[0]:
                scales[0] = low
            ...
            for j in range(1, self.J):
                ...
                # Do the level 2+ transform
                low, h = FWD_J2PLUS_3D.apply(low, self.h0a, self.h1a, self.h0b,
                                      self.h1b, self.skip_hps[j], self.o_dim,
                                      self.ri_dim, mode)
                highs[j] = h
                if self.include_scale[j]:
            ...
            raise ValueError("Only 2D and 3D input supported")
